    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <Animatable.View animation="fadeIn" style={styles.headerIconContainer}>
          <View style={[styles.bellCircle, { backgroundColor: colors.primary + "15" }]}>
            <MaterialIcons name="notifications" size={45} color={colors.primary} />
          </View>
        ...
        {/* --- SETTINGS GROUP --- */}
        <Animatable.View animation="fadeInUp" delay={200} style={[styles.groupContainer, { backgroundColor: colors.card }]}>
          {notificationItems.map((item, index) => (
            <View 
              key={item.id} 
              style={[
                styles.row, 
                index !== notificationItems.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }
              ]}
            >
...
const styles = StyleSheet.create({
  ...
  bellCircle: { width: 90, height: 90, borderRadius: 45, justifyContent: 'center', alignItems: 'center', marginBottom: 15 },
  headerSubtitle: { textAlign: 'center', fontSize: 14, lineHeight: 20, paddingHorizontal: 30 },

  groupContainer: { borderRadius: 32, paddingVertical: 10, elevation: 4, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 15 },
  ...
  tipBox: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 30 },
