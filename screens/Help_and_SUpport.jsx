...
export default function HelpSupport() {
  ...
  const contactOptions = [
    { icon: "email", label: "Email Support", subtitle: "support@silentvoice.app", action: () => Linking.openURL("mailto:support@silentvoice.app") },
    { icon: "menu-book", label: "User Guide", subtitle: "Learn how to use every feature", action: () => navigation.navigate("SeeAll") },
  ];
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60 }}>
        <Text style={[styles.groupTitle, { color: colors.subtext }]}>Contact Us</Text>
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          {contactOptions.map((item, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.row,
                index !== contactOptions.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border },
              ]}
              onPress={item.action}
            >
        ...
        <Text style={[styles.groupTitle, { color: colors.subtext, marginTop: 22 }]}>
...
const styles = StyleSheet.create({
  ...
  faqQuestion: { fontSize: 14, fontWeight: "700", flex: 1, marginRight: 10 },
  faqAnswer: { fontSize: 13, lineHeight: 20, paddingHorizontal: 16, paddingBottom: 16, fontWeight: "500" },
});