      ...
      >
        ...
        >
          ...
          <TouchableOpacity 
            ...
            activeOpacity={0.7}
            style={[styles.themeToggle, { backgroundColor: isDarkMode ? "#334155" : "#F1F5F9" }]}
          >
            <MaterialIcons 
        ...
        <View style={styles.iconRow}>
          {[
            ...
            <Animatable.View key={index} animation="zoomIn" delay={index * 100} style={styles.iconWrapper}>
              <Pressable
                onPress={() => navigation.navigate(item.route)}
                style={({ pressed }) => [
                  styles.iconBox, 
                  { backgroundColor: colors.card, transform: [{ scale: pressed ? 0.92 : 1 }] }
                ]}
              >
                {item.lib === "FA5" ? (
                ...
                <Text style={[styles.iconText, { color: colors.text }]}>{item.label}</Text>
              </Pressable>
            </Animatable.View>
          ))}
        </View>
        ...
        <View style={styles.sectionHeader}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>Popular Tools</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 15 }}>
          <LinearGradient colors={["#6366F1", "#8B5CF6"]} style={styles.toolCard}>
          ...
          <LinearGradient colors={["#F43F5E", "#FB7185"]} style={styles.toolCard}>
            <View style={styles.cardIconCircle}>
        ...
        <Animatable.View 
          animation="fadeInUp" 
          delay={1000} 
          style={[styles.tipCard, { backgroundColor: colors.primary + "15", borderColor: colors.primary + "30" }]}
        >
          <MaterialIcons name="lightbulb" size={20} color={colors.primary} />
          <Text style={[styles.tipText, { color: colors.text }]}>
            Did you know? Consistent practice in the Quiz section improves sign retention by 40%!
          </Text>
        </Animatable.View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  themeToggle: { width: 44, height: 44, borderRadius: 14, justifyContent: "center", alignItems: "center" },
  ...
  sectionHeader: { marginTop: 35, flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end" },
  sectionTitle: { fontSize: 18, fontWeight: "800", letterSpacing: -0.5 },
  viewAll: { fontSize: 14, fontWeight: "700" },

  toolCard: { width: width * 0.45, height: 190, borderRadius: 28, padding: 20, justifyContent: "flex-end", marginRight: 15, elevation: 6 },
  cardIconCircle: { width: 45, height: 45, borderRadius: 15, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center', position: 'absolute', top: 20, left: 20 },
  cardTitle: { color: "#fff", fontSize: 16, fontWeight: "800" },
  cardSubtitle: { color: "#fff", fontSize: 11, opacity: 0.8, marginTop: 2 },

  tipCard: { marginTop: 35, padding: 20, borderRadius: 20, borderWidth: 1, flexDirection: 'row', alignItems: 'flex-start' },
